...
class LosApiController < ApplicationController
  ...
  def api
    ...
    options = params.permit(:id_europeana, lo_profile: [:title, :description, :language, :year, :id_europeana, :url, :thumbnail_url])
    ...
    #     options[:lo_profile] = storedLoProfile.recursive_symbolize_keys unless storedLoProfile.blank?
    #   end
    # else
